...
const TaskListTable = ({ tableData }) => {
    const getStatusBadgeColor = (status) => {
        switch (status) {
            ...
            case 'In Progress': return 'bg-purple-100 text-purple-500 border border-purple-200';
            case 'Pending': return 'bg-cyan-100 text-cyan-500 border border-cyan-200';
    ...
    return (
        <div className="overflow-x-auto p-0 rounded-lg mt-3">
            <table className="min-w-full">
                ...
                <tbody>
                    {tableData.map((task) => (
                        <tr key={task._id} className="border-t border-gray-200 hover:bg-blue-50/50 transition-colors duration-200 cursor-pointer">
                            <td className="my-3 mx-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden">{task.title}</td>
                            <td className="py-4 px-4">
                                <span className={`px-2 py-2 text-xs rounded inline-block transition-all duration-200 hover:scale-105 ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                            </td>
                            <td className="py-px-4">
                                <span className={`px-2 py-2 text-xs rounded inline-block transition-all duration-200 hover:scale-105 ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                            </td>
